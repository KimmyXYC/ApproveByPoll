...
class JoinRequest:
    ...
    async def handle_join_request(self, bot, request: types.ChatJoinRequest, db):
        ...
        notice_message = await bot.send_message(
            self.chat_id,
            f"{self.user_mention} is requesting to join this group.",
        ...
        if vote_message.total_voter_count == 0:
            ...
            approve_user = False
            edit_msg = f"{self.user_mention}: No one voted."
            ...
            approve_user = True
            edit_msg = f"{self.user_mention}: Approved."
            ...
            approve_user = False
            edit_msg = f"{self.user_mention}: Denied."
    ...
    async def handle_button(self, bot, callback_query: types.CallbackQuery, action):
        ...
        if action == "Approve":
            ...
            edit_msg = f"{self.user_mention}: Approved by {admin_mention}"
            ...
            edit_msg = f"{self.user_mention}: Denied by {admin_mention}"
            ...
            if chat_member.can_restrict_members or chat_member.status == 'creator':
                if self.bot_member.status == 'administrator' and self.bot_member.can_restrict_members:
                    ...
                    edit_msg = f"{self.user_mention}: Banned by {admin_mention}"
                    reply_msg = "您的申请已被拒绝。\nYour application have been denied."
                else:
                    self.finished = True
                    approve_user = False
